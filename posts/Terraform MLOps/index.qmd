...
The typical ML pipeline's lifecycle consists of model training, registration, deployment, monitoring and decommissioning. All of these steps are typically reflected as changes in infrastructure, if nothing else because they all usually run on the cloud: even if scale doesn't force you to do this, legal constraints around data usually will.

On the other hand, Terraform is one the most popular tools for infrastructure-as-code, it helps Engineers track and automate the lifecycle of cloud infrastructure and operations. So it is worth asking whether Terraform could orchestrate all of the ML operations. But what would it look like, and is it a good idea?
...
3. Creation of a batch compute environment in Fargate or EC2 (configurable), along with queues and task definitions
...
2. Containerise a specific model from the MLFlow Registry (this can happen locally) and upload it to its ECR
...
The deployment logic would check for [active projects in SSM](#tracking-active-projects), and throw an error if the deployment job references a project which hasn't been trained before.
...
2. The list of active projects in SSM is updated based on the referenced project.

3. `terraform apply` is called
...
As the number of managed projects grow, there will be a need for using project-specific configuration and hardware, e.g. provisioning GPUs for deep learning projects. While this set up does not allow for this out of the box, it is easy to incorporate. In the working prototype, each project folder can have its own `training-resource-requirements.json`, which is passed directly as AWS Batch configuration.

This also creates the need to have a default configuration, which can be incorporated in the training job's module.
